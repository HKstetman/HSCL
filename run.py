...
from trains.singleTask.model import hscl
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    if args.is_distill:
        ...
        model = []
        model_hscl = getattr(hscl, 'HSCL')(args)
        model_hscl = model_hscl.cuda()

        model = [model_hscl]
    else:
        print("testing phase for HSCL")
        model = getattr(hscl, 'HSCL')(args)
        model = model.cuda()
    ...
    if args.mode == 'test':
        model.load_state_dict(torch.load('pt/4.pth'))
        results = trainer.do_test(model, dataloader['test'], mode="TEST")
        sys.stdout.flush()
        input('[Press Any Key to start another run]')
    else:
        epoch_results = trainer.do_train(model, dataloader, return_epoch_results=from_sena)
        model[0].load_state_dict(torch.load('pt/hscl.pth'))
